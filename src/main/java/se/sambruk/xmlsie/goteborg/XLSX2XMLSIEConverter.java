package se.sambruk.xmlsie.goteborg;
...
public class XLSX2XMLSIEConverter {
  ...
  public void convertFromCSV(Reader reader, Writer output) throws Exception {
    ...
    while ((line = br.readLine()) != null) {
      ...
      if (columns.length != 7) {
        log.warn("Rows must contain exactly 7 columns! Error on data line " + lineNumber + "\n" + line);
        continue;
//        throw new RuntimeException("Rows must contain exactly 7 rows! Error on data line " + lineNumber);
      }
      ...
      int kontonummer = Integer.valueOf(columns[4]);
      String kontonamn = columns[5] == null ? "" : columns[5].trim();
      ...
      try {
        BigDecimal amount = new BigDecimal(columns[6].replaceAll(",", ""));
    ...
    sie.getCompany().setOrganizationalnumber("212000-1355");
    sie.getCompany().setName("Göteborgs kommun");
    sie.getCompany().setAddressLine1("Köpmansgatan 20");
    sie.getCompany().setPostcode("411 13");
    sie.getCompany().setCity("Göteborg");
    sie.getCompany().setCountryCode("SE");
    sie.getCompany().setHomepage("http://goteborg.se/");
